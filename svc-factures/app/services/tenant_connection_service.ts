...
export function connectionNameFor(serviceId: number): string {
  return `tenant_factures_${serviceId}`
}
...
/**
 * Enregistre (si besoin) la connexion Lucid d'un tenant. add() est un
 * no-op documenté si has() est déjà vrai — sûr à appeler à chaque requête.
 */
...
// Message exact levé par pg-pool (`node_modules/pg-pool/index.js`) quand un
// pool a été fermé (close()) mais reste enregistré côté Lucid — has()
// resterait vrai, donc ensure() seul ne suffit pas à s'en remettre.
 ...
 * retente une seule fois — jamais de deuxième retry, une erreur après ça
 * est une vraie panne.
 */
  ...
  try {
    return await tenantConnectionStorage.run(name, async () => {
      try {
        ...
        // release() retire l'enregistrement (contrairement à close() seul)
        // -- ensure() va donc bien recréer le pool, pas juste réutiliser
        // l'entrée fermée.
...
/**
 * Résout les connexions tenant de tous les services `factures` d'un
 * organisme — fan-out borné (1 à quelques bases), jamais l'annuaire
 * entier. Utilisé par verify()/staffIndex.
 */
...
/**
 * Exécute `fn` sur CHAQUE service factures connu, tous organismes
 * confondus — réservé aux endpoints AREGIE qui n'ont structurellement
 * aucun organisme à filtrer (pendingCollection/acknowledgeCollection,
 * voir aregie_controller.ts). Concurrence plafonnée : le nombre de bases
 * tenant croît avec le nombre de services, pas de `Promise.all` non
 * borné sur l'annuaire entier.
 */
