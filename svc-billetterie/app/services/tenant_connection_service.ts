...
export function connectionNameFor(serviceId: number): string {
  return `tenant_billetterie_${serviceId}`
}
...
/**
 * Résout les connexions tenant de tous les services billetterie d'un
 * organisme — fan-out borné, jamais l'annuaire entier. Utilisé pour les
 * vues admin (staffIndex, stats() côté admin) où l'agent voit tout
 * l'organisme.
 */
 ...
 * l'agent.serviceIds d'un JWT, déjà filtré par permission) — fan-out
 * borné à ce que l'appelant connaît déjà, pas de lecture de l'annuaire.
 *
 * serviceIds vient du JWT interne, une liste TOUS TYPES DE SERVICE
 * CONFONDUS (un agent peut avoir accès à des services billetterie ET
 * inscription à la fois) — un id qui n'est pas un service billetterie
...
/**
 * Exécute `fn` sur CHAQUE service billetterie connu, tous organismes
 * confondus — réservé aux jobs sans organisme à filtrer (retry mail).
 * Concurrence plafonnée.
 */
