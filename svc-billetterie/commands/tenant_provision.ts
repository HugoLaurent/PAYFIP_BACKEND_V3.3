...
export default class TenantProvision extends BaseCommand {
  ...
  async run() {
    ...
    // billetterie en local) — pas de nouveau conteneur par tenant. dbName
    // n'est jamais interpolé depuis une entrée libre : il vient
    // uniquement de ce gabarit.
    const dbName = `svc_billetterie_tenant_${serviceId}`
    ...
    this.logger.info(
      `Vérification de la base ${dbName} sur ${connectionConfig.host}:${connectionConfig.port}...`
    )
