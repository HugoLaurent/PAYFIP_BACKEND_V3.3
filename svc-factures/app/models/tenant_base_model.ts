...
/**
 * Base commune aux modèles vivant dans une base par service (Invoice,
 * FailedInvoiceMail) : chaque méthode statique de requête/écriture lit la
 * connexion tenant active (AsyncLocalStorage, posée par
 * tenant_connection_service.runOnTenant) et la passe explicitement à
 * Lucid — jamais de mutation de la connexion par défaut globale, qui
 ...
 * service (query/create/find/findBy/updateOrCreate) — pas toute la
 * surface de LucidModel. Les casts sont nécessaires : TypeScript ne
 * propage pas `this: T` d'une méthode statique à travers un appel
 * `super.xxx()` générique, même si l'implémentation reste correcte au
 * runtime (this reste bien le sous-type appelant, comme tout `super` sur
